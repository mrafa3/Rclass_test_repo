...
all voting justices by issue area. For instance, if you explored the previous question, you found that Scalia voted "liberal" on almost 70% of Federal Taxation cases. But does the court "lean" one way or another based on the type of case? This question is a bit more complicated, but does Scalia *lean liberal* based on the Court's tendencies in Federal Taxation Cases? (*This question still uses `dplyr` -- and if you'd like, `ggplot2` -- but with a few bells and whistles.*)

```{r baseline_vote_fedtax, include=TRUE, echo=TRUE, eval=TRUE, fig.height=7}
baseline_vote <- sup_court %>% 
  ...
  summarise(baseline_issue_vote = round(mean(liberal_vote, na.rm = TRUE) * 100, 1)) %>% 
...
sup_court_fedtax %>% 
  ...
  annotate(geom='text', x=50, y='SSotomayor', label='Baseline vote for Federal Taxation cases', size=5) + 
  scale_x_continuous(limits = c(0, 100)) + 
  ggtitle('Percent of Federal Taxation cases with liberal decision by Justice since 1980',
          subtitle = "The Court leans liberal on Federal Taxation cases") + 
  labs(x='Percent of cases with liberal decision',
       y='',
       color='',
