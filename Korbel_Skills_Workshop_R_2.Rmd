...
#note: to truly answer questions such as this, you would want to take into account the directionality of 
#all voting justices. Do we know whether a 69% for Federal Taxation cases makes Scalia left-leaning?
```

*  Since 1980, how have all Supreme Court justices voted along *The Supreme Court Database's* liberal/conservative coding schema across issue areas?

```{r liberal_lean_1980, include=TRUE, echo=TRUE, eval=FALSE, fig.height=11}
sup_court %>% 
  filter(term > 1980) %>% 
  group_by(justiceName, issueAreaName, directionName) %>% 
  summarise(count_cases = n()) %>% 
  ...
  geom_point(aes(x=perc_cases,
                 y=reorder(issueAreaName, perc_cases)), color='blue', size=3) +   # Draw points
  geom_vline(xintercept = 50) + 
  ggtitle('Percent of cases with liberal ruling by Justice and Issue Area',
          subtitle = 'Since 1980') +
  labs(y='',
       x='Percent of cases with liberal ruling',
       caption="Source: Washing University at St. Louis\nThe Supreme Court Database") + 
  theme_minimal() + 
...
#this is a small multiple plot, which is often useful for exploring these kinds of questions
#facet_wrap() makes this pretty simple
```
