---
title: "Korbel Skills Workshop: Data Manipulation and Visualization Basics using R"
author: "Mickey Rafa"
date: "May 15, 2018"
fontsize: 12pt
output:
...
```{r gapminder_histogram_1, include=TRUE, echo=TRUE, eval=TRUE}
gapminder %>%                           #DATA
...
```{r summary_ggplot_viz1, include=TRUE, echo=TRUE, eval=FALSE}
gapminder_by_year_continent %>% 
  ggplot(aes(x=year,
             y=meanlifeExp,
             group=continent,
...
There are many, many ways that you can customize or expand your visualization repertoire by understanding the three basic elements of ggplot graphics:  
...
Here is a look at two other extensions (*you can just follow along*):  
...
```{r library_ggthemes, include=TRUE, echo=TRUE}
...
```{r mult_geoms, include=TRUE, echo=TRUE, eval=TRUE, fig.height=6.2}
gapminder %>% 
...
To demonstrate the joining functions of `dplyr`, we'll use a classic example with very small datasets that are pre-loaded in RStudio.[^11] Let's start by printing the `band_members` dataset (`print(band_members)`).

[^11]: This example is used in dplyr's documentation on joins, found here: https://dplyr.tidyverse.org/reference/join.html
