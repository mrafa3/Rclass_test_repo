...
`dplyr` is designed to standardize the data management and manipulation process in R. `dplyr` has a small set of functions (which may be more easily thought of as verbs) which are designed to help users navigate the most common challenges of data manipulation.[^2] These are:  
...
We will use R's `ggplot2` package to create exploratory visualizations of data. This will give you the building blocks for creating beautiful visualizations. There are three common elements of ggplot graphics that you will learn how to use in this session, including:  
...
We'll learn the main `dplyr` variables using the **gapminder** dataset. Now that we have our project environment set up, some brief refreshers from the previous workshop:  
...
Let's look at the **distinct continents** in the gapminder dataset. In your code chunk, relace `distinct(year)` with `distinct(continent)`.
...
Without piping, you need to write this code inside-out! Does anyone actually find this easier?! You might think of piping as the Earnest Hemingway style of writing code in R (simple, yet powerful).
...
Earnest Hemingway in *The Sun Also Rises*
...
![Here for obvious reasons](https://upload.wikimedia.org/wikipedia/en/0/02/Phantomtollbooth.PNG)
...
```{r gapminder_scatter_1, include=TRUE, echo=TRUE, eval=TRUE}
gapminder %>%                           #DATA
  filter(year == 2007) %>%              #DATA
  ggplot(aes(x=gdpPercap,               #AESTHETICS
                 y=lifeExp)) +          #AESTHETICS
  geom_point()
  geom_smooth(method = "lm") #GEOM
```
...
*  Within ggplot's functions (`ggplot()` and `geom_point()`) we use a plus sign. Think of ggplot as a canvas where the graphic is built by the addition of elements.  
*  Then, we defined the aesthetics (`aes()`), or what we wanted to graph.  
*  Finally, we added the **geom** to the aesthetic, which tells ggplot the type of graphic that we wanted to create.  
...
```{r gapminder_scatter_2_2, include=TRUE, eval=FALSE, echo=TRUE}
gapminder %>%                         #DATA
  filter(year == 2007) %>%            #DATA
  ggplot(aes(x=gdpPercap,             #AESTHETIC
             y=lifeExp)) +            #AESTHETIC    
  geom_point(color="blue")            #GEOM
```
...
```{r summary_ggplot_viz1_2, include=TRUE, echo=TRUE, eval=FALSE}
gapminder_by_year_continent %>% 
  ggplot(aes(x=year,
             y=meanlifeExp)) + 
  geom_line()
...
```{r zambia_life_expectancy, include=TRUE, echo=TRUE, eval=FALSE}
...
There are many other `dplyr` functions and `ggplot2` customizations that we couldn't cover. 
...
```{r sup_court_q1, include=TRUE, eval=FALSE}
sup_court %>% 
  group_by(chief) %>% 
...
```{r sup_court_q2_1, include=TRUE, eval=FALSE}
sup_court %>% 
...
```{r sup_court_q2_2, include=TRUE, eval=FALSE}
sup_court %>% 
  filter(justiceName == 'NMGorsuch') %>% 
...
```{r sup_court_q2_3, include=TRUE, eval=FALSE}
sup_court %>% 
  filter(justiceName == 'NMGorsuch') %>% 
...
```{r sup_court_q3, include=TRUE, eval=FALSE}
sup_court %>% 
  filter(majVotes == 5,
         minVotes == 4) %>% 
  group_by(justiceName, majorityName) %>% 
...
```{r sup_court_q4, include=TRUE, eval=FALSE}
sup_court %>% 
  group_by(justiceName) %>% 
  summarise(count_cases = n()) %>% 
  arrange(desc(count_cases))
...
```{r sup_court_term_issue_df, include=TRUE, eval=FALSE}
...
```{r sup_court_issue_history_viz1, include=TRUE, eval=FALSE, fig.height=7}
...
```{r sup_court_issue_history_viz2, include=TRUE, eval=FALSE, fig.height=7}
sup_court_term_issue %>% 
  filter(!is.na(issueAreaName)) %>% 
...
(**Note: anticipating that we won't have enough time to cover joins in this session.**)

Joining datasets might be one of the most valuable data wrangling skills that you can learn. More often than not, the data that you want to analyze don't live in a single dataframe, but are across two more more datasets. Joining datasets correctly and carefully helps ensure the integrity of the data.

To demonstrate the joining functions of `dplyr`, we'll use a classic example with very small datasets that are pre-loaded in RStudio.[^10] Let's start by printing the `band_members` dataset (`print(band_members)`).

[^10]: This example is used in dplyr's documentation on joins, found here: https://dplyr.tidyverse.org/reference/join.html

```{r print_band_members, include=TRUE, echo=TRUE}
print(band_members)
```

Let's say that, in this fictituous analysis, we want to explore the instruments played by band members. Print the `band_instruments` dataset.

```{r print_band_instruments, include=TRUE, echo=FALSE}
print(band_instruments)
```

You can see that `band_members` and `band_instruments` share a column, or variable -- **name**. We want to combine these tiny datasets to see the instruments played in the Beatles and the Stones.

We'll learn three types of joins in this session, but there are several more that serve specific purposes. We will explore the use of the three most common joins:  

*  **Inner joins**: a join that merges datasets, keeping only the common values of the two datasets (`dplr::inner_join()`).  
*  **Left joins**: a join the merges datasets, keeping all the values in the "left" or 1st dataset and only the values in the "right" or 2nd dataset that have matching values in the left dataset (`dplyr::left_join()`).  
*  **Anti joins**: a join that returns all unmatched values from the left dataset. This join is typically used for troubleshooting join issues and evaluating mismatched values in the two datasets (`dplr::anti_join()`).  

<center>

![Common data joins and their meaning](http://www.dofactory.com/Images/sql-joins.png)

</center>

Each of dplr's join functions (including the above) follow a similar structure and take the same arguments:  

*  **x**: dataframe #1, or the "left" dataset,  
*  **y**: dataframe #2, or the "right" dataset,  
*  **by**: the variable or column that you want to join on. This can take multiple variables, as well as variables with different names.  

First, let's do an inner join on `band_members` and `band_instruments`. These two datasets share a common variable -- **name** -- which will be the **by=** argument. 

```{r inner_join_ex1, include=TRUE}
inner_join(x=band_members,                     #dataframe 1
           y=band_instruments,                 #dataframe 2
           by='name')                          #variable to join dataframe 1 & 2 on (note: quotes required)
```

Our dataset included members from The Beatles and The Rolling Stones. So why did the inner join only return members of The Beatles?  Hint -- look closely at the names.

What if we wanted to keep all records from the `band_members` dataset, but still join on `band_instruments`? 

```{r left_join_ex1, include=TRUE}
left_join(x=band_members,                     #dataframe 1
           y=band_instruments,                 #dataframe 2
           by='name')                          #variable to join dataframe 1 & 2 on (note: quotes required)
```

Because Mick Jagger is not in the `band_instruments` dataset, the **plays** column from the `left_join()` returns NA.

Now, if you wanted to see which band members are not in both datasets, an `anti_join()` is very handy. What would you expect to see from an `anti_join()` on these two datasets?

```{r anti_join_ex1, include=TRUE}
anti_join(x=band_members,                      #dataframe 1
           y=band_instruments,                 #dataframe 2
           by='name')                          #variable to join dataframe 1 & 2 on (note: quotes required)
```

In each of these join examples, the **by=** argument remained the same. However, it is likely that the two datasets that you want to join do not share the *exact same* variable names to join on. Let's try a left join using `band_members` as the "left" dataset and `band_instruments2` as the "right" dataset. Before we try this join, we need to first look at the `band_instruments2` dataset.  

```{r print_band_instruments2, include=TRUE, echo=FALSE}
print(band_instruments2)
```

In `band_instruments2`, the two variables are now **artist** and **plays** (name is no longer a column name). So, let's tell `left_join()` that **name** and **artist** are the same variable, just with different column names.

```{r left_join_ex2, include=TRUE, echo=TRUE}
left_join(x=band_members,                   #dataframe 1
          y=band_instruments2,              #dataframe 2
          by=c('name' = 'artist'))           #variable to join dataframe 1 & 2 on (note: variables in list c())
```